#VADER STOCK SENTIMENT ANALYSIS
...
def analyze_stock_sentiment(ticker, months_back, api_limit, api_key):
    try:
        ...
        return "Error: Months back must be a number.", None, None
    ...
    try:
        ...
        return f"Yahoo Finance Error: {e}", None, None
    
    if history_df.empty:
        return "No stock data found.", None, None
    ...
    if error:
        return f"API Error: {error}", None, None
    ...
    if news_list:
        ...
        for article in news_list:
            ...
            capt= f"{article.get('title', '')}. {article.get('summary', '')}"
            scores= analyzer.polarity_scores(capt)
            scored_news.append({'Date': date_object, 'Sentiment': scores['compound']})
...
def run_analysis():
    ...
    result_text.set(f"VADER Analysis: {t_symbol}\nData Points: {len(df)}\nCorrelation: {corr:.4f}")
